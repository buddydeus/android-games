...
- International Chess version `0.0.6` uses standard square placement, complete special moves and draw rules, legal-move-equivalent threefold-repetition keys, player-score-driven 1-10 offline search, explicit draw session state, Xiangqi-family score/undo/restart behavior, and a 180-degree Black-player view with upright piece glyphs. Player tap resolution always regenerates candidates from the current position after robot replies so subsequent moves and captures cannot use a stale previous-turn move list. Its square board and right rail remain side by side at 800×600 landscape, and the rail distinguishes the active side, check, terminal result, AI level, undo, and restart.
