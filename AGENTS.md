...
The Dou Shou Qi module owns its immutable 7x9 initial board, orthogonal movement, Rat-only river access, Rat-blocked Lion/Tiger jumps, rank captures, Rat/Elephant exception, terrain-bound Rat captures, trap weakening, immutable move application, and the exact den/final-capture/no-move/repetition/100-quiet terminal precedence. Its local rule variant allows both sides to enter, occupy, and leave their own den without winning; keep public rules and primitive search move generation equivalent while opponent-den entry remains terminal. Keep these rules and their board/movement/capture/state/terminal tests together in `games/doushouqi/`.
