...
- Keep Junqi's authoritative hidden `JunqiState` private to `JunqiSession`. Public session projections may expose only the active observer's `JunqiObservation`. `HANDOFF` must expose no board, deployment, rank, or last-move coordinates. `BATTLE_RESULT` may additionally expose the observer-known last move and generic battle outcome so the board remains visible, but never an opponent's true rank.
...
- Keep Junqi UI projections observation-only: green-side (internal `BLUE`) bottom display mapping rotates coordinates 180 degrees without rotating board or piece labels, opponents remain backs except a publicly revealed flag, `HANDOFF` mounts no board or piece semantics, and `BATTLE_RESULT` renders the observer-safe board with a generic side-rail judgement.
