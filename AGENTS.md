...
- Junqi Task 2 owns the approved immutable 12x5 board graph and legal movement foundation: keep all 60 nodes, road/rail adjacency, read-only camps and headquarters, engineer railway turns, straight-only non-engineer railway movement, occupied-camp attack protection, and immobility rules inside `games/junqi/`.
...
| `games/junqi/` | Junqi package module, manifest metadata, immutable board graph, and legal movement tests |
