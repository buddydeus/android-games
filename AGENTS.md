...
- Keep Junqi UI projections observation-only: green-side (internal `BLUE`) bottom display mapping rotates coordinates 180 degrees without rotating board or piece labels, opponents remain backs except a publicly revealed flag, and `HANDOFF`/`BATTLE_RESULT` must mount no board or piece semantics beneath their fully opaque pages.
