...
- **斗兽棋**：标准 7×9 松绿/朱砂棋盘，完整支持鼠入河、狮虎跃河与鼠阻挡、兽阶吃子、鼠象例外、陷阱降阶和进入对方兽穴。终局按入穴、吃光、无棋可走、三次重复、连续 100 个半回合未吃子依次判定。离线 1–10 级确定性迭代加深 Alpha-Beta AI 按玩家累计胜分升级，后台结果必须匹配会话世代与源局面；1–5 级使用确定性弱化。`0.0.2` 包含一张 1400×1400 透明角竹木棋盘和松绿/朱砂双方共 16 张 512×512 透明浮雕棋子，运行时按包版本缓存并逐张校验 PNG 类型与尺寸，单张无效时回退到 Compose 绘制。棋盘保持近方形“双河峡谷”，侧栏继续沿用六款游戏统一的象棋家族几何；玩家执朱砂时旋转坐标而保持兽名正向。
