...
- [五子棋](games/gomoku/README.md) `0.0.6`
- [黑白棋](games/othello/README.md) `0.0.6`
- [象棋](games/xiangqi/README.md) `0.0.13`
- [国际象棋](games/chess/README.md) `0.0.10`
- [军棋](games/junqi/README.md) `0.0.13`
- [斗兽棋](games/doushouqi/README.md) `0.0.9`
...
| `games/doushouqi/` | 斗兽棋标准规则、确定性 1–10 级 AI、会话、Compose 界面和包资源 |
| `scripts/` | 本地调试启动脚本 |
| `docs/` | 架构设计、实施计划和游戏包开发说明 |

游戏包通过 `DexClassLoader` 加载。外壳不包含具体棋类规则，也不硬编码游戏名称、Logo 或排列序号：名称来自清单的 `displayName`，Logo 来自清单 `icon` 指向的包内资源，首页按各游戏成功启动次数从高到低排列。相同次数按名称和 `gameId` 保持稳定顺序。横屏游戏选择区固定使用 4 列网格，最后一行不足 4 个时从第一列开始排列并保留右侧空槽；紧凑或竖屏布局继续使用单列按钮。因此单个游戏的功能更新或问题修复可以通过替换对应游戏包完成，导入兼容 zip 后无需升级游戏中心。
...
每款游戏独立维护版本并从 `0.0.1` 开始；当前斗兽棋为 `0.0.9`，五子棋和黑白棋为 `0.0.6`，国际象棋为 `0.0.10`，军棋和象棋为 `0.0.13`。游戏开始界面会显示该游戏包的 `versionName`。修改某款游戏的规则、机器人、界面或包内资源时，只递增该游戏的 `versionCode` 和语义化 `versionName`，并保持插件代码与 `package/manifest.json` 中的版本一致；仅修改游戏中心外壳不需要调整游戏版本。

六款游戏的横屏宽布局统一以象棋为几何基准：页面外边距 `28dp`，棋盘与右侧栏间距 `34dp`；对局侧栏固定宽 `300dp`、占内容高度 `94%`，菜单侧栏固定宽 `320dp`、占内容高度 `88%`。棋盘在剩余区域居中并保持各自原始纵横比，低于 `900dp` 的可用宽度时改为棋盘在上、侧栏在下，各游戏的棋盘绘制、状态信息和专属操作保持不变。

游戏包契约、清单字段和新增游戏步骤见 [游戏插件开发说明](docs/agents/game-plugins.md)。

## 当前游戏能力

六款游戏均提供独立开始界面、单人模式、本地双人对战、对局状态、历史比分，以及右侧栏中的悔棋和重新开始操作。每次有效行动后，棋盘使用与棋子保持明显间隔的半透明亮蓝色四角框标记最后落点；标记限制在当前单元格内且不遮挡棋子中心。单人模式中机器人响应后标记机器人落点，交换为后手方的新局会标记机器人的自动首步。单人模式悔棋会回退玩家与机器人组成的一轮，双人模式悔棋回退一步，并同步恢复此前的最后落点；胜者产生后隐藏悔棋按钮，和局仍保留悔棋。撤销终局时比分同步恢复，重新开始后清空本局悔棋历史。单人模式中，玩家获胜后重新开始会交换执方；玩家失败后重新开始固定恢复先手方。和局重新开始保持当前执方，双人模式仍由固定先手方开局。
...
- **军棋**：12×5 二国暗棋棋盘，支持合法布阵、完整碰子和终局规则、单人及本地双人隐私交接。离线 1–10 级 AI 只接收观察者可见信息与知识状态，不读取敌方真实军衔；它在专用后台执行器中运行，结果必须匹配会话世代才能应用。碰子判定不再阻塞对局或要求确认：单人模式侧栏分别保留最近一次我方发起和对方发起的碰子结果，双人模式在不透明交接后显示刚发生的结果；胜方阵营色后附带“进攻/防守”，每条记录只显示当前观察者自己的碰子棋子，不公开敌方军衔。界面采用传统铁路军棋盘的明亮现代复刻：麦秆金完整棋盘包含矩形兵站、椭圆行营、红字大本营、黑白枕木铁路和三条中央桥路；橙方与绿方棋子使用整面阵营色和尽量放大的白色黑体军衔。包内提供军棋 Logo、完整棋盘和开始界面陈列纹理，并对缺失或无效位图提供 Compose 后备绘制。
- **斗兽棋**：标准 7×9 松绿/朱砂棋盘，完整支持鼠入河、狮虎跃河与鼠阻挡、兽阶吃子、鼠象例外、陷阱降阶和进入对方兽穴；本地规则允许双方棋子进入、停留和离开己方兽穴，但不会因此获胜。任意棋子停在任意陷阱中都按零级防守，敌方任意棋子均可吃它，包括象吃鼠；棋子离开陷阱攻击时恢复原兽阶。终局按进入对方兽穴、吃光、无棋可走、三次重复、连续 100 个半回合未吃子依次判定。离线 1–10 级确定性迭代加深 Alpha-Beta AI 按玩家累计胜分升级，后台结果必须匹配会话世代与源局面；1–5 级使用确定性弱化。`0.0.9` 包含一张 1400×1400 透明角竹木棋盘、松绿/朱砂双方共 16 张 512×512 透明浮雕棋子，以及用户选定的透明象鼠 S 河冷瓷 Logo；运行时按包版本缓存并逐张校验 PNG 类型与尺寸，单张无效时回退到 Compose 绘制。棋盘保持近方形“双河峡谷”，六个陷阱使用带向心网线、倒钩、中央锁结和浅赭印底的双层八角捕兽网纹章。游戏开始侧栏统一为 Logo、标题、版本和三项操作；单人对局侧栏的当前回合与终局阵营只显示 `绿方` / `红方`，不显示玩家执方。单人侧栏汇总上一轮完整“玩家走棋 + 智能应答”中的最多两枚吃子；双人侧栏在每次合法走棋后立即替换为该步的一枚吃子，普通走棋立即显示 `无吃子`。玩家执朱砂时旋转坐标而保持兽名正向。
...
- [斗兽棋 Logo 与侧栏实施计划](docs/superpowers/plans/2026-07-27-doushouqi-logo-sidebar.md)
- [斗兽棋 Logo 与侧栏运行验收](docs/superpowers/reports/2026-07-27-doushouqi-logo-sidebar-runtime-acceptance.md)
- [斗兽棋单人最近一步吃子设计](docs/superpowers/specs/2026-07-28-doushouqi-single-player-capture-summary-design.md)
- [斗兽棋单人最近一步吃子实施计划](docs/superpowers/plans/2026-07-28-doushouqi-single-player-capture-summary.md)
- [斗兽棋单人最近一步吃子运行验收](docs/superpowers/reports/2026-07-28-doushouqi-single-player-capture-summary-runtime-acceptance.md)
- [斗兽棋完整回合吃子摘要设计](docs/superpowers/specs/2026-07-28-doushouqi-round-capture-summary-design.md)
- [斗兽棋完整回合吃子摘要实施计划](docs/superpowers/plans/2026-07-28-doushouqi-round-capture-summary.md)
