...
- [斗兽棋](games/doushouqi/README.md) `0.0.3`
...
每款游戏独立维护版本并从 `0.0.1` 开始；当前斗兽棋为 `0.0.3`，五子棋和黑白棋为 `0.0.6`，国际象棋为 `0.0.10`，军棋和象棋为 `0.0.13`。游戏开始界面会显示该游戏包的 `versionName`。修改某款游戏的规则、机器人、界面或包内资源时，只递增该游戏的 `versionCode` 和语义化 `versionName`，并保持插件代码与 `package/manifest.json` 中的版本一致；仅修改游戏中心外壳不需要调整游戏版本。
...
- **斗兽棋**：标准 7×9 松绿/朱砂棋盘，完整支持鼠入河、狮虎跃河与鼠阻挡、兽阶吃子、鼠象例外、陷阱降阶和进入对方兽穴。终局按入穴、吃光、无棋可走、三次重复、连续 100 个半回合未吃子依次判定。离线 1–10 级确定性迭代加深 Alpha-Beta AI 按玩家累计胜分升级，后台结果必须匹配会话世代与源局面；1–5 级使用确定性弱化。`0.0.3` 包含一张 1400×1400 透明角竹木棋盘和松绿/朱砂双方共 16 张 512×512 透明浮雕棋子，运行时按包版本缓存并逐张校验 PNG 类型与尺寸，单张无效时回退到 Compose 绘制。棋盘保持近方形“双河峡谷”，六个陷阱使用带向心网线、倒钩、中央锁结和浅赭印底的双层八角捕兽网纹章；侧栏继续沿用六款游戏统一的象棋家族几何，玩家执朱砂时旋转坐标而保持兽名正向。
