...
**Iteration:** 2026-07-28T16:00:00+08:00 — 双人即时吃子摘要修订
...
12. **Capture-summary copy revision** - 单人模式不显示 `松绿`、`朱砂` 或 `玩家执…`；所有阵营文字只使用 `绿方` / `红方`。单人按“玩家 + 智能”显示上一完整轮最多两枚吃子。双人不合并双方走棋，每次合法走棋都立即替换为该步的一枚 `绿方吃：<兽名>` / `红方吃：<兽名>`，普通走棋立即显示 `无吃子`。
...
颜色不单独承担信息：单人模式阵营同时显示 `绿方`、`红方` 文本，双人模式保留完整阵营名；河道、陷阱和兽穴同时依赖形状、位置与汉字标识。
...
│  empty 7×9 territory board   │   版本 0.0.7     │
│      blue river canyon       │──────────────────│
│  traps · den                 │   [单人模式]     │
│                              │   [双人对战]     │
│                              │   [退出游戏]     │
...
- 单人模式回合区只显示 `绿方` / `红方`，不显示玩家执方。其下按绿方、红方顺序显示上一完整“玩家走棋 + 智能应答”轮的最多两行 `绿方吃：<兽名>` / `红方吃：<兽名>`，整轮无人吃子显示 `无吃子`；智能思考期间保留上一完整轮。双人模式显示上一合法走棋的一枚吃子；每步即时替换，未吃子立即显示 `无吃子`，不等待另一方也不合并两步。
...
- 胜负状态保留最终局面；单人右栏显示 `绿方进入兽穴`、`红方无棋可走` 等明确原因，双人模式可继续使用完整阵营名。
- 支持系统字体缩放；读屏顺序为局面摘要、当前回合、比分、可用操作。每枚棋子的语义包含阵营、兽名和坐标。

## Copy Tone

- Register: 简短、明确，像棋盘旁的裁判记录。
- Vocabulary: 单人模式的回合方和终局阵营只使用 `绿方` / `红方`，不出现 `松绿`、`朱砂` 或 `玩家执…`。吃子摘要使用 `绿方吃：<兽名>`、`红方吃：<兽名>`、`无吃子`；其余使用 `选择棋子`、`悔棋`、`重新开始`。
- Empty/error state: 单人上一完整轮或双人上一合法走棋没有发生吃子时显示 `无吃子`；其他错误说明原因和下一步，例如 `当前棋子没有合法走法，请选择另一枚棋子`。
