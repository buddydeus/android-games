...
**Iteration:** 2026-07-28T10:00:00+08:00 — 单人回合与吃子摘要修订
...
12. **Round-capture copy revision** - 单人模式不显示 `松绿`、`朱砂` 或 `玩家执…`；所有阵营文字只使用 `绿方` / `红方`。单双人对局都显示上一完整轮的吃子摘要：单人按“玩家 + 智能”、双人按“绿方 + 红方”完成一轮，使用 `绿方吃：<兽名>` / `红方吃：<兽名>`，整轮无人吃子显示 `无吃子`。正常半轮保留上一完整轮，首步直接终局时立即发布。
...
│  empty 7×9 territory board   │   版本 0.0.6     │
│      blue river canyon       │──────────────────│
│  traps · den                 │   [单人模式]     │
│                              │   [双人对战]     │
│                              │   [退出游戏]     │
...
- 单人模式回合区只显示 `绿方` / `红方`，不显示玩家执方。单双人模式都在其下显示上一完整轮摘要：有吃子时按绿方、红方顺序显示最多两行 `绿方吃：<兽名>` / `红方吃：<兽名>`，整轮无人吃子显示 `无吃子`。单人智能思考期间和双人等待红方走棋期间继续显示上一完整轮；若首步直接终局，则立即显示该步摘要。
...
- Empty/error state: 上一完整轮没有发生吃子时显示 `无吃子`；其他错误说明原因和下一步，例如 `当前棋子没有合法走法，请选择另一枚棋子`。
