...
12. **Single-player copy revision** - 单人模式不显示 `松绿`、`朱砂` 或 `玩家执…`；所有阵营文字只使用 `绿方` / `红方`。原玩家执方位置改为“最近一步吃子”摘要，只展示最近一次玩家或智能走棋直接吃掉的一枚棋子；最近一步未吃子时显示 `无`。
...
- 单人模式回合区只显示 `绿方` / `红方`，不显示玩家执方。其下使用一行紧凑摘要显示 `最近一步吃子`；只列出最近一次玩家或智能走棋直接吃掉的一枚棋子，并用红/绿文字标签同时表达所属方与兽名。最近一步没有吃子时显示 `无`，不保留更早的吃子记录。
...
- Vocabulary: 单人模式的回合方和终局阵营只使用 `绿方` / `红方`，不出现 `松绿`、`朱砂` 或 `玩家执…`。其余使用 `最近一步吃子`、`选择棋子`、`悔棋`、`重新开始`。
- Empty/error state: 最近一步没有发生吃子时显示 `最近一步吃子：无`；其他错误说明原因和下一步，例如 `当前棋子没有合法走法，请选择另一枚棋子`。
