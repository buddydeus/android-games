...
为五子棋、黑白棋、象棋、国际象棋和斗兽棋建立一套可独立随游戏包更新的圆形 Logo。游戏中心仅读取清单 `icon` 指向的 PNG，不包含任何棋种识别或绘制逻辑。军棋继续遵循其独立视觉 SSOT，但保持相同的包内尺寸和圆形安全边界。
...
- **斗兽棋:** 用户选定的松绿 `象` 棋与朱砂 `鼠` 棋隔 S 形蓝釉河对峙；棋子使用真实方形浮雕字棋，冷矿物灰瓷章外圈与其他游戏相同。不得改写字形、河形、棋位、材质或光照，只移除章外白底。

## Asset Rules

- 五子棋、黑白棋、象棋和国际象棋不使用文字、字母或数字；斗兽棋仅保留实际棋子上的 `象`、`鼠` 单字。所有 Logo 均不使用边框外装饰、水印和品牌标记。
- 主体必须完整落在圆形徽章内，缩小到 64dp 后仍能区分。
- 不使用透明度表达关键轮廓；黑白主体必须有清晰边界。
- 输出保留完整方形 PNG，游戏中心的圆形裁切负责隐藏四角。
- 斗兽棋 PNG 必须使用 RGBA 透明外底，四角 alpha 为 `0`；瓷章边缘不得残留白边或键色。
...
| 斗兽棋 | `designs/previews/android-game-logo-doushouqi.png` | `games/doushouqi/package/assets/icon.png` |

## Acceptance

- 五个本规范覆盖的 PNG 均可解码、尺寸一致且为正方形。
- 五个游戏 zip 均包含 `assets/icon.png`，不再依赖 `icon.txt`。
- 首页名称仍来自 `displayName`，排列仍由成功启动频率决定。
- 仅更新实际改动的游戏版本；游戏中心壳版本保持不变。
