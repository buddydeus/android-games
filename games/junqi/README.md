...
- `HANDOFF` 继续使用独立、完全不透明的 Compose 页面，不挂载棋盘或棋子语义节点。碰子不再进入独立阻塞阶段，也没有确认按钮；正常对局侧栏以橙/绿阵营色标配合 `橙方胜`、`绿方胜` 或 `同归于尽` 显示结果，并在胜利、失败或同归于尽时都显示 `我方棋子：<军衔>`，不显示敌方军衔。
