...
- `HANDOFF` 继续使用独立、完全不透明的 Compose 页面，不挂载棋盘或棋子语义节点。碰子不再进入独立阻塞阶段，也没有确认按钮；正常对局侧栏显示最近一次 `进攻方胜`、`防守方胜` 或 `同归于尽`，不显示敌方军衔。
...
- 双人模式分别完成橙绿双方布阵，并在每步后用不含棋盘、最后移动或军衔数据的不透明交接状态切换观察者。碰子后无需确认并直接进入下一方交接；接管棋盘后仅显示双方都已知的通用裁决，不公开敌方军衔。
