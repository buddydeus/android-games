...
- `HANDOFF` 继续使用独立、完全不透明的 Compose 页面，不挂载棋盘或棋子语义节点。碰子不再进入独立阻塞阶段，也没有确认按钮；单人模式以 `上回合我方碰子结果`、`上回合对方碰子结果` 分别保留最近一次玩家发起和机器人发起的碰子摘要，双人模式仅显示刚发生的一条。胜方文字使用 `橙方胜 - 进攻`、`绿方胜 - 防守` 等阵营与身份组合，同归于尽使用橙绿双色标；每条记录都以放大的粗体显示 `我方棋子：<军衔>`，不显示敌方军衔。
...
- `JunqiSession` 完整覆盖 `DEPLOYMENT`、`HANDOFF`、`PLAYING` 和 `FINISHED`。布阵支持合法双选交换、种子随机、重置和准备；橙方（内部 `RED`）始终先行。
- 双人模式分别完成橙绿双方布阵，并在每步后用不含棋盘、最后移动或军衔数据的不透明交接状态切换观察者。碰子后无需确认并直接进入下一方交接；接管棋盘后只发布胜方阵营和接管方自己的碰子棋子名称，不公开另一方军衔。
