...
`doushouqi` 是游戏中心的第六个内置独立游戏包，版本为 `0.0.6`。规则、AI、会话、Compose 界面和包内资源都归本模块所有。
...
- 单人模式按“玩家走棋 + 智能应答”、双人模式按“绿方走棋 + 红方应答”汇总上一完整轮的吃子；正常半轮不会覆盖已完成摘要，首步直接终局时立即发布。每轮按捕获方显示最多两行 `绿方吃：<兽名>` / `红方吃：<兽名>`，整轮无人吃子显示 `无吃子`；悔棋恢复对应轮次边界，重新开始清空。
...
- 宽屏沿用 28dp 外边距、34dp 间距与 300dp/320dp 侧栏；菜单侧栏将透明 Logo、标题和版本置于上区，三项操作置于下区。对局侧栏按比分、回合/结果、操作分为三段并使用两条分隔线。单人侧栏的当前回合和终局阵营只显示 `绿方` / `红方`，不显示松绿、朱砂或玩家执方；单双人侧栏都显示上一完整轮的 `绿方吃：<兽名>` / `红方吃：<兽名>`，无人吃子显示 `无吃子`。窄于 900dp 时切换为棋盘在上、操作栏在下。
